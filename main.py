...
def init_db():
    ...
    try:
        ...
        # ---------------------------------------------------
        # BANNER TABLE (for the home page sliding banner)
        # Matches the table exactly as it exists in phpMyAdmin:
        # `Banner` (capital B) with columns bid, rid, title, restaurant,
        # images.
        #
        # IMPORTANT (updated): the table now carries BOTH
        #   - `rid`         -> the restaurant's real numeric id, a proper
        #                      FK into restaurants(rid). This is the
        #                      column the app should use to resolve which
        #                      restaurant a banner points to.
        #   - `restaurant`  -> the restaurant's display NAME as plain
        #                      text (e.g. "johnmarcos"), kept only so the
        #                      name can be shown under the banner title
        #                      without an extra join.
        # Every lookup/validation below is keyed off `rid`, never off the
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS `Banner` (
                ...
                images VARCHAR(255) NOT NULL,
                CONSTRAINT fk_banner_restaurant
                FOREIGN KEY (rid)
                REFERENCES restaurants(rid)
                ON DELETE CASCADE
                ON UPDATE CASCADE
...
# =========================================================
# BANNERS (home page sliding window / carousel)
# Table: Banner(bid, rid, title, restaurant, images)
# `rid` is the real, FK-validated link to restaurants.rid and is what
# the frontend uses to jump to a restaurant. `restaurant` is only the
# restaurant's display NAME, kept for convenience so the app can show
# it as a subtitle without an extra lookup.
# =========================================================
...
async def add_banner(
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        # Validate rid against restaurants.rid (the reliable numeric key)
        # and pull the display name to store alongside it.
        cursor.execute("SELECT rid, rname FROM restaurants WHERE rid=%s", (rid,))
        restaurant = cursor.fetchone()
        if not restaurant:
            raise HTTPException(status_code=400, detail="Restaurant not found")

        restaurant_name = restaurant["rname"]

        saved_filename, file_path = save_uploaded_file(images, "banner")
        content = await images.read()
        with open(file_path, "wb") as buffer:
            buffer.write(content)

        cursor.execute(
            "INSERT INTO `Banner` (rid, title, restaurant, images) VALUES (%s, %s, %s, %s)",
            (rid, title, restaurant_name, saved_filename)
        )
        conn.commit()
...
def get_banners():
    ...
    cursor = db.cursor(dictionary=True)
    try:
        # `rid` is returned so the app can resolve the target restaurant
        # directly, without relying on a fuzzy name match.
        cursor.execute("""
            SELECT bid, rid, title, restaurant, images
            FROM `Banner`
            ORDER BY bid ASC
        """)
        rows = cursor.fetchall()
...
@app.put("/api/banner/{bid}")
@app.post("/update_banner/{bid}")
async def update_banner(
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        cursor.execute("SELECT * FROM `Banner` WHERE bid=%s", (bid,))
        existing = cursor.fetchone()
        if not existing:
            raise HTTPException(status_code=404, detail="Banner not found")

        new_title = title.strip() if title and title.strip() else existing["title"]
        new_rid = existing["rid"]
        new_restaurant_name = existing["restaurant"]
        current_image = existing["images"]
        ...
        if rid is not None:
            ...
            if not restaurant:
                raise HTTPException(status_code=400, detail="Restaurant not found")
            new_rid = restaurant["rid"]
            new_restaurant_name = restaurant["rname"]
        ...
        cursor.execute(
            "UPDATE `Banner` SET title=%s, rid=%s, restaurant=%s, images=%s WHERE bid=%s",
            (new_title, new_rid, new_restaurant_name, current_image, bid)
        )
        conn.commit()
        ...
        return {
            ...
            "rid": new_rid,
            "title": new_title,
            "restaurant": new_restaurant_name,
            "images": f"{BASE_MEDIA_URL}{current_image}"
        }
    finally:
        cursor.close()
...
@app.delete("/delete_banner/{bid}")
@app.delete("/api/banner/{bid}")
async def delete_banner(bid: int):
    conn = get_db()
    cursor = conn.cursor(dictionary=True)
    try:
        cursor.execute("SELECT images FROM `Banner` WHERE bid=%s", (bid,))
        row = cursor.fetchone()
        if row and row["images"]:
            path = os.path.join(IMAGES_DIR, os.path.basename(row["images"]))
            if os.path.exists(path):
        ...
        cursor.execute("DELETE FROM `Banner` WHERE bid=%s", (bid,))
        conn.commit()
        return {"status": "success", "deleted": bid}
    finally:
        cursor.close()
        conn.close()
